 ...
 */
public class PoiExcelExportAndImport {
    ...
    public PoiExcelExportAndImport(){

    }

    public PoiExcelExportAndImport(String excelPath){
     ...
     * @return
     */
    public PoiExcelExportAndImport RestoreSettings(){
        PoiExcelExportAndImport instance = new  PoiExcelExportAndImport(this.excelPath);
        return instance;
    }
   ...
   /* public List<Row> readExcel(String xlsPath) throws IOException{
        ...
        String ext = xlsPath.substring(xlsPath.lastIndexOf(".")+1);//获取扩展名
    ...
    public void writeExcel_xls(List<Row> rowList, String src_xlsPath, String dist_xlsPath) throws IOException {
        ...
        try {
            ...
            File file = new File(dist_xlsPath); // 判断文件是否存在
            if (file.exists()) {
                // 如果复写，则删除后
                if (isOverWrite) {
                    file.delete();
